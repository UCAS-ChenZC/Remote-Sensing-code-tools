...
PATH = '/home/solid/my_tools/Rolabel脚本/RoLabelImg_Transform-master'
...
fr = open(os.path.join(PATH + '/RoLabelImg_Transform/txt_to_xml_list.txt'))
...
for line in fr.readlines():
    if line:
        ...
        img = Image.open(os.path.join(img_path, img_base_name + '.png'))
        create_xml = CreateXml(foldername='pixeliinkimg',
                               filename=line.strip().split('.')[0],
                               imgSize=img.size,
                               databaseSrc='Unknown',
                               localImgPath=os.path.join(img_path, img_base_name + '.png'))
        ...
        for bbox_line in fr_line.readlines():
            ...
            name = ""
            for x in bbox_list[8:-1]:
                name = name + x
            create_xml.addRotatedBndBox(cx, cy, w, h, angle, name, 0)
