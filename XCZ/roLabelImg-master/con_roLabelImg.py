...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def saveLabels(self, annotationFilePath):  # 保存label到指定位置
    ...
    def scrollRequest(self, delta, orientation):
        ...
        #添加修改为int
        bar.setValue(bar.value() + bar.singleStep() * int(units))

    def setZoom(self, value):
        self.actions.fitWidth.setChecked(False)
        self.actions.fitWindow.setChecked(False)
        self.zoomMode = self.MANUAL_ZOOM
        # self.zoomWidget.setValue(value) change
        self.zoomWidget.setValue(int(value))

    def addZoom(self, increment=10):
        # self.setZoom(self.zoomWidget.value() + increment) change
        self.setZoom(self.zoomWidget.value() + int(increment))  # 将 increment 转换为整数

    def zoomRequest(self, delta):
        units = delta / (8 * 15)
        scale = 10
        # self.addZoom(scale * units) change
        self.addZoom(int(scale * units))  # 将 scale * units 转换为整数
    ...
    def loadFile(self, filePath=None):
        ...
        # 如果没有指定文件路径，则从设置中获取最后打开的文件路径
        if filePath is None:
            filePath = self.settings.get('filename')

        unicodeFilePath = ustr(filePath)
        # Tzutalin 20160906 : Add file list and dock to move faster
        # 如果文件路径有效且文件列表控件中有项目，则高亮显示文件项
        if unicodeFilePath and self.fileListWidget.count() > 0:
            index = self.mImgList.index(unicodeFilePath)
            fileWidgetItem = self.fileListWidget.item(index)
            fileWidgetItem.setSelected(True)

        # 如果文件路径有效且文件存在，则尝试加载文件
    ...
    def finding(self, filename):
        if self.defaultSaveDir is not None and len(ustr(self.defaultSaveDir)):
            imgFileName = os.path.basename(filename)  # frame3022_009856.png
            savedFileName = os.path.splitext(imgFileName)[0] + XML_EXT  # frame3022_009856.xml
            xmlPath = os.path.join(ustr(self.defaultSaveDir), savedFileName)
        else:
            imgFileDir = os.path.dirname(filename)  # 文件目录路径
            imgFileName = os.path.basename(filename)  # 文件名 eg file.png
            savedFileName = os.path.splitext(imgFileName)[0] + XML_EXT  # file.xml
            xmlPath = os.path.join(imgFileDir, savedFileName)
        return xmlPath

    def openNextImg(self, _value=False):

        # Proceding next image without dialog if having any label
        # 如果设置了自动保存，并且有默认的保存目录，且当前图像有未保存的更改，则保存当前图像
        if self.autoSaving is True and self.defaultSaveDir is not None:
            if self.dirty is True:
                self.dirty = False  # 将dirty标志设置为False，表示没有未保存的更改
                self.canvas.verified = True  # 将canvas的verified属性设置为True，表示图像已验证
                self.saveFile()  # 调用saveFile方法保存文件
        ...
        # 初始化filename变量，用于存储下一个要打开的图像文件名
        filename = None
        pre_filename = None  # 在外部初始化pre_file
        # 如果当前没有打开的文件，则从图像列表的第一个文件开始
        if self.filePath is None:
            filename = self.mImgList[0]
        else:
            # 否则，找到当前文件在列表中的索引，并尝试打开下一个文件
            currIndex = self.mImgList.index(self.filePath)
            # 检查是否还有下一个文件
            if currIndex + 1 < len(self.mImgList):
                ...
                filename = self.mImgList[currIndex + 1]
        ##############################################################################################################
        if pre_filename is not None:
            pre_xmlPath = self.finding(pre_filename)
            # print('666',pre_xmlPath)
            xmlPath = self.finding(filename)
            # print('777',xmlPath)
            # 如果下一张图像不存在xml文件，复制前一张图像xml给下一张
            if os.path.exists(xmlPath):
                pass
            elif not os.path.exists(pre_xmlPath):
                pass
            else:
                shutil.copyfile(pre_xmlPath, xmlPath)
        ##############################################################################################################
        # 如果找到了下一个文件，则加载它
        if filename:
            self.loadFile(filename)
    ...
    def saveFile(self, _value=False):
        if self.defaultSaveDir is not None and len(ustr(self.defaultSaveDir)):
            if self.filePath:
                ...
                savedPath = os.path.join(ustr(self.defaultSaveDir), savedFileName)  # 默认存储路径
                self._saveFile(savedPath)
        else:
            imgFileDir = os.path.dirname(self.filePath)  # 文件目录路径
            imgFileName = os.path.basename(self.filePath)  # 文件名 eg file.png
            savedFileName = os.path.splitext(imgFileName)[0] + XML_EXT  # file.xml
