...
class PascalVocReader:
    ...
    # for Fairm
    def addRotatedShape_FAIR1M(self, label, robndbox, difficult):
        for index,ax in enumerate(robndbox):
            if index == 0:
                p0x, p0y = ax.text.split(',')
            if index == 1:
                p1x, p1y = ax.text.split(',')
            if index == 2:
                p2x, p2y = ax.text.split(',')
            if index == 3:
                p3x, p3y = ax.text.split(',')
        # p0x,p0y = self.rotatePoint(cx,cy, cx - w/2, cy - h/2, -angle)
        # p1x,p1y = self.rotatePoint(cx,cy, cx + w/2, cy - h/2, -angle)
        # p2x,p2y = self.rotatePoint(cx,cy, cx + w/2, cy + h/2, -angle)
        # p3x,p3y = self.rotatePoint(cx,cy, cx - w/2, cy + h/2, -angle)
        ...
        points = [(p0x, p0y), (p1x, p1y), (p2x, p2y), (p3x, p3y)]
        self.shapes.append((label, points, True, None, None, difficult))
    ...
    def parseXML(self):
        ...
        #增加source
        filename = xmltree.find('source').find('filename').text
        try:
        ...
        for object_iter in xmltree.findall('objects')[0]:
            # typeItem = object_iter.find('type')

            # print(typeItem.text)
            # if typeItem.text == 'bndbox':
            # bndbox = object_iter.find("bndbox")
            # label = object_iter.find('possibleresult').find('name').text
            # # Add chris
            # difficult = False
            ...
            robndbox = object_iter.find('points')
            label = object_iter.find('possibleresult').find('name').text
            difficult = False
            # if object_iter.find('difficult') is not None:
            #     difficult = bool(int(object_iter.find('difficult').text))
            ...
            #new
            self.addRotatedShape_FAIR1M(label, robndbox, difficult)
