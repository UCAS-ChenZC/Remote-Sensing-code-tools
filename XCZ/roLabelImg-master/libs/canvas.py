...
class Canvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.current is not None and len(self.line) == 2:
            ...
            p.setBrush(brush)
            p.drawRect(int(leftTop.x()), int(leftTop.y()), int(rectWidth), int(rectHeight))#转int
            
            #draw dialog line of rectangle
            p.setPen(self.lineColor)
            p.drawLine(int(leftTop.x()),int(rightBottom.y()),int(rightBottom.x()),int(leftTop.y()))#转int
